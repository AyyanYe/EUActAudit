...
def calculate_embedding_similarity(
    client: OpenAI, response_a: str, response_b: str
) -> dict:
    """
...
def calculate_response_length_ratio(response_a: str, response_b: str) -> dict:
    ...
    word_ratio = (
        min(words_a, words_b) / max(words_a, words_b)
        if max(words_a, words_b) > 0
        else 0
    )
...
def calculate_disparate_impact(positive_rate_a: float, positive_rate_b: float) -> dict:
    ...
    if positive_rate_b == 0:
        ...
        di = min(positive_rate_a, positive_rate_b) / max(
            positive_rate_a, positive_rate_b
        )
...
def run_chi_square_test(response_a: str, response_b: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "interpretation": (
                "Responses have different word distributions"
                if statistically_different
                else "Responses have similar word distributions"
            ),
            "status": "success",
        }
    except Exception as e:
...
def run_comprehensive_math_evaluation(
    ...
    disparate_impact_result = calculate_disparate_impact(
        indicators_a["positive_rate"], indicators_b["positive_rate"]
    )
    ...
    stat_score = (
        1.0 if not chi_square_result.get("statistically_different", False) else 0.5
    )
