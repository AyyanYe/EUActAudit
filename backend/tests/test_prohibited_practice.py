...
def test_emotion_recognition_ban():
    ...
    if (
        ...
        or "wait" in response_text
        or "banned" in response_text
        or "article 5" in response_text
    ):
    ...
    if data.get("facts", {}).get("exemption_probe_answered") == "no":
        ...
        print(
            f"[WARNING] 'exemption_probe_answered' is '{data.get('facts', {}).get('exemption_probe_answered', 'NOT FOUND')}'"
        )

    # Check if bot stopped asking compliance questions
    response_text = data.get("response", "").lower()
    if (
        "assessment halted" in response_text
        or "cannot proceed" in response_text
        or "illegal" in response_text
    ):
        print("[SUCCESS] Bot issued prohibited practice block message")
    else:
        print(
            "[WARNING] Bot may still be asking compliance questions instead of blocking"
        )

    # Check if bot is NOT asking about human oversight or data
    if (
        "human oversight" in response_text
        or "data" in response_text
        or "compliance" in response_text
    ):
        if (
            "cannot" in response_text
            or "halted" in response_text
            or "illegal" in response_text
        ):
            print("[SUCCESS] Bot is blocking, not asking compliance questions")
        else:
            print(
                "[FAILURE] Bot is still asking compliance questions after UNACCEPTABLE determination"
            )
    else:
