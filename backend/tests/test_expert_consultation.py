...
def test_compute_stuck_on_topic_first_gap():
    ...
    assert (
        compute_stuck_on_topic(topic_ask_count, fact_dict, "HIGH") == "human_oversight"
    )
...
def test_normalize_compliance_facts_low_confidence():
    ...
    data = {
        "human_oversight": "we have something",
        "confidence_scores": {"human_oversight": 40},
    }
    normalize_compliance_facts(data)
    assert data["human_oversight"] == "partial_or_unclear"
...
def test_compute_stuck_on_topic_data_governance():
    ...
    assert (
        compute_stuck_on_topic(topic_ask_count, fact_dict, "HIGH") == "data_governance"
    )
...
def test_compute_topic_ask_count_mixed_logs():
    ...
    logs = [
        ...
        Log(
            "bot",
            "Article 10 requires data governance. Can you implement bias testing?",
        ),
