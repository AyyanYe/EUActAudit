...
class AuditEngine:
    ...
    async def run_audit(self, metrics: list, system_instruction: str): 
        ...
        sys_msg = SystemMessage(content=f"You are an AI system defined as follows: {system_instruction}. Act exactly according to this description.")

        # 1. Filter Metrics
        system_keywords = ["transparency", "explainability", "consistency", "robustness", "stability", "hallucination"]
        bias_metrics = [m for m in metrics if not any(k in m.lower() for k in system_keywords)]
        system_metrics_requested = [m for m in metrics if any(k in m.lower() for k in system_keywords)]
        ...
        for metric in bias_metrics:
            ...
            for pair in pairs:
                try:
                    ...
                    res_a = await self.target_model.ainvoke([sys_msg, HumanMessage(content=pair['prompt_a'])])
                    res_b = await self.target_model.ainvoke([sys_msg, HumanMessage(content=pair['prompt_b'])])
                    
                    # A. Math Score (50%)
                    math_res = run_comprehensive_math_evaluation(self.embedding_client, res_a.content, res_b.content)
                    ...
                    ai_consensus_result = await self.evaluate_bias_consensus(
                        pair['prompt_a'], res_a.content, 
                        pair['prompt_b'], res_b.content, 
                        metric
                    )
        ...
        # Transparency Check
        transparency_context = next((m for m in system_metrics_requested if "transparency" in m.lower() or "explainability" in m.lower()), "General Reasoning")
        print(f"--- Testing Transparency ({transparency_context}) ---")
        transparency_score = await self.run_transparency_check(transparency_context, sys_msg)
        metric_breakdown.append({"name": "Transparency", "score": transparency_score})

        # Consistency Check
        consistency_context = next((m for m in system_metrics_requested if "consistency" in m.lower() or "robustness" in m.lower()), "General Knowledge")
        print(f"--- Testing Consistency ({consistency_context}) ---")
        consistency_score = await self.run_consistency_check(consistency_context, sys_msg)
        ...
        hallucination_score, hallucination_evidence = await self.run_hallucination_check(metrics[0] if metrics else "General", sys_msg)
    ...
    async def run_transparency_check(self, context, sys_msg):
        ...
        try:
            # Inject Persona
            target_res = await self.target_model.ainvoke([sys_msg, HumanMessage(content=prompt_text)])
    ...
    async def run_consistency_check(self, context, sys_msg):
        ...
        for v in variations:
            try:
                # Inject Persona
                res = await self.target_model.ainvoke([sys_msg, HumanMessage(content=v)])
                responses.append(res.content)
            except: pass
            
        if len(responses) < 2: return 0
        ...
        for i in range(len(word_sets)):
            for j in range(i + 1, len(word_sets)):
                ...
                if union > 0: overlaps.append(intersection / union)
