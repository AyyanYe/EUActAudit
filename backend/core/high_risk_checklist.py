...
def get_missing_mandatory_topics(
    facts: Dict[str, str], obligations: List[dict]
) -> List[str]:
    """
    ...
    for topic in HIGH_RISK_MANDATORY_TOPICS:
        if topic == "human_oversight":
            ...
            if has_oversight_obligation and oversight_value in [
                "present",
                "planned",
                "yes",
            ]:
...
def can_complete_high_risk_assessment(
    facts: Dict[str, str], obligations: List[dict]
) -> bool:
    """
