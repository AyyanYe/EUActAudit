...
def create_compliance_cert(data: dict) -> bytes:
    ...
    elements.append(
        Paragraph(
            ...
            ParagraphStyle(
                "Disclaimer", parent=small_style, alignment=TA_JUSTIFY, fontSize=8
            ),
        )
    )
    ...
    if obligations_list:
        ...
        met_ob = sum(
            ...
            if (ob.get("status") or "").strip().lower()
    ...
    if len(profile_rows) > 1:
        ...
        elements.append(
            Paragraph("<i>No profile facts were collected.</i>", body_style)
        )
    elements.append(Spacer(1, 10))
    ...
    elements.append(PageBreak())
    elements.append(
        Paragraph("3. Article-by-Article Compliance Analysis", heading_style)
    )
            ...
            if topic_key:
                ...
                if rag_text:
                    ...
                    if len(rag_text) > 500:
                        truncated += "..."
                    elements.append(
                        Paragraph("<b>What the law requires:</b>", bold_body)
                    )
                    elements.append(
                        Paragraph(
    ...
    if gaps:
        ...
        for idx, ob in enumerate(gaps, 1):
            ...
            title = ob.get("title", code)
            topic_key = OB_CODE_TO_TOPIC.get(code, "")
            priority, timeline = TOPIC_PRIORITY.get(
                topic_key, ("MEDIUM", "Medium-term")
            )
    ...
    elements.append(
        Paragraph(
            "Generated by EUActAudit | EU AI Act Compliance Platform",
            small_style,
        )
    )
...
def _generate_obligation_chart(obligations: list) -> io.BytesIO | None:
    ...
    for i, (val, status_name) in enumerate(
        zip(score_values, [_status_label(ob.get("status", "")) for ob in obligations])
    ):
