...
def evaluate_compliance_state(facts: dict):
    ...
    is_prohibited_emotion_recognition = facts.get(
        "purpose"
    ) == "emotion_recognition" and facts.get("context") in [
        "workplace",
        "education",
        "school",
    ]
    ...
    is_prohibited_biometric = (
        facts.get("purpose") == "biometric_id"
        and facts.get("biometric_mode") == "real_time"
        and facts.get("context") == "public_space"
    )

    # If prohibited practice detected, check if exemption probe has been answered
    if (
        is_prohibited_emotion_recognition
        or is_prohibited_social_scoring
        or is_prohibited_biometric
    ):
        ...
        if exemption_answered is None:
            if is_prohibited_emotion_recognition:
                return (
                    ...
                    [],
                    [
                        "EXEMPTION_PROBE_NEEDED: Emotion recognition in education/workplace requires exemption check."
                    ],
                )
                ...
                return (
                    ...
                    [],
                    [
                        "EXEMPTION_PROBE_NEEDED: Social scoring requires exemption check."
                    ],
                )
                ...
                return (
                    ...
                    [],
                    [
                        "EXEMPTION_PROBE_NEEDED: Real-time biometric ID requires exemption check."
                    ],
                )
        ...
        if exemption_answered == "no":
            if is_prohibited_emotion_recognition:
                context_type = (
                    "school"
                    if facts.get("context") in ["education", "school"]
                    else "workplace"
                )
                return (
                    "UNACCEPTABLE",
    ...
    high_risk_domains = [
        "recruitment",
        "hr",
        "worker_management",  # Annex III (4)
        "education",
        "vocational_training",  # Annex III (3)
        "critical_infrastructure",  # Annex III (2)
        "credit_scoring",
        "insurance",  # Annex III (5) - Essential Services
        "biometrics",  # Annex III (1) - Post-remote ID
        "law_enforcement",
        "migration",
        "justice",  # Annex III (6,7,8)
    ]
    ...
    if facts.get("domain") in high_risk_domains:
            ...
            [
                {
                    ...
                    "desc": "System must meet accuracy, robustness, and cybersecurity requirements under Article 15.",
                },
                {
                    "code": "ART_12",
                    "title": "Record Keeping",
                    "desc": "System must automatically log operations and decisions under Article 12.",
                },
            ]
        )
        ...
        if role == "provider" or role == "builder":
                ...
                [
                    {
                        ...
                        "desc": "Implement a QMS compliant with Art 17.",
                    },
                    {
                        "code": "ART_43",
                        "title": "Conformity Assessment",
                        "desc": "Must undergo conformity assessment before deployment.",
                    },
                    {
                        "code": "ART_10",
                        "title": "Data Governance",
                        "desc": "Training data must be relevant, representative, and free of errors.",
                    },
                ]
            )
            # Providers must DESIGN systems with human oversight capability (Article 14).
            if (
                facts.get("human_oversight") in ["no", "absent", "partial"]
                or facts.get("automation") == "fully_automated"
            ):
                warnings.append(
                    "Article 14: Providers must design high-risk systems to allow effective human oversight by deployers."
                )
                obligations.append(
                    {
                ...
                [
                    {
                        ...
                        "desc": "Ensure human overseers are trained and have authority to stop the system.",
                    },
                    {
                        "code": "ART_26_LOGS",
                        "title": "Monitoring & Logging",
                        "desc": "Keep logs of operation for at least 6 months.",
                    },
                    {
                        "code": "ART_10",
                        "title": "Data Governance",
                        "desc": "Input data must be relevant and representative for the system's intended purpose.",
                    },
                ]
            )
            ...
            if (
                ...
                warnings.append(
                    "CRITICAL: Article 14 requires human oversight for high-risk systems. Fully automated decisions without human review are non-compliant."
                )
                obligations.append(
                    {
                        "code": "ART_14_OVERSIGHT",
            ...
            warnings.append(
                "Role unclear: Are you building this (Provider) or buying it (Deployer)?"
            )

    # --- LEVEL 3: LIMITED RISK (Transparency - Art 50) ---

    elif (
        facts.get("capability") == "chatbot" or facts.get("capability") == "interaction"
    ):
        risk_level = "LIMITED"
        obligations.append(
            {
                "code": "ART_50",
                "title": "Transparency Notification",
                "desc": "Users must be informed they are talking to an AI.",
            }
        )

    elif facts.get("capability") == "content_generation" and facts.get(
        "media_type"
    ) in ["image", "video", "audio"]:
        risk_level = "LIMITED"
        obligations.append(
            {
                "code": "ART_50_WATERMARK",
                "title": "AI Watermarking",
                "desc": "Output must be machine-readable as AI generated.",
            }
        )
