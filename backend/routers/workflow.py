...
def create_workflow(
    ...
    return {
        ...
        "created_at": (
            new_workflow.created_at.isoformat() if new_workflow.created_at else None
        ),
    }
        ...
        "workflows": [
            {
                ...
                .filter(InterviewLog.workflow_id == wf.id)
                .count(),
            }
            for wf in workflows
        ]
...
def get_workflow(
    ...
    workflow = (
        db.query(Workflow)
        .filter(Workflow.id == workflow_id, Workflow.project_id == project_id)
        .first()
    )
    ...
    logs = (
        ...
        .filter(InterviewLog.workflow_id == workflow_id)
