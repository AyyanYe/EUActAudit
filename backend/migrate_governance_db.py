...
def migrate_governance_schema():
    ...
    try:
        ...
        if "interview_state" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE projects ADD COLUMN interview_state TEXT DEFAULT 'INIT'"
            )
            # Update existing rows
            cursor.execute(
                "UPDATE projects SET interview_state = 'INIT' WHERE interview_state IS NULL"
            )
            conn.commit()
        ...
        if "confidence_level" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE projects ADD COLUMN confidence_level TEXT DEFAULT 'LOW'"
            )
            # Update existing rows
            cursor.execute(
                "UPDATE projects SET confidence_level = 'LOW' WHERE confidence_level IS NULL"
            )
            conn.commit()
